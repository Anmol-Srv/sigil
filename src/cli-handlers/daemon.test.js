...
describe('daemon restart lifecycle', () => {
  it('delegates to the OS supervisor when automatic start owns the daemon', async () => {
    const restartService = vi.fn().mockResolvedValue({ ok: true, manager: 'launchd' });
    const stop = vi.fn();
    const start = vi.fn();
  ...
  it('fails clearly when the managed service cannot restart', async () => {
    await expect(restartDaemonLifecycle({
      isServiceInstalled: vi.fn().mockResolvedValue(true),
      restartService: vi.fn().mockResolvedValue({ ok: false, manager: 'launchd' }),
      stop: vi.fn(),
      start: vi.fn(),
