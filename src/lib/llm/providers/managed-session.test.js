...
import { chat, laneForCaller } from './managed-session.js';
...
describe('managed-session provider', () => {
  it('isolates interactive, ingest, and maintenance workloads', () => {
    expect(laneForCaller('query-router')).toBe('claude:interactive');
    expect(laneForCaller('extractor')).toBe('claude:ingest');
    expect(laneForCaller('relation-namer')).toBe('claude:maintenance');
  });

  it('routes to manager.submit and maps the result to the chat() contract', async () => {
    const calls = [];
    setSessionManager({
      hasUsableWorkers: (t) => t === 'claude:ingest',
      submit: async (task) => { calls.push(task); return { text: '{"facts":[]}', inputTokens: 12, outputTokens: 3, model: 'haiku', cost: 0, workerId: 'claude-0', reqId: 'req-1', viaFallback: false, queuedMs: 7 }; },
    });
    ...
    expect(calls[0]).toMatchObject({ sourceType: 'claude:ingest', prompt: 'extract from "hi"', model: 'haiku', caller: 'extractor' });
    // Correlation fields (workerId/reqId/viaFallback) flow back through to logCall.
    expect(r).toEqual({ text: '{"facts":[]}', inputTokens: 12, outputTokens: 3, model: 'haiku', cost: 0, workerId: 'claude-0', reqId: 'req-1', viaFallback: false, queuedMs: 7 });
  });
