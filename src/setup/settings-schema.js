...
export const SETTINGS_SECTIONS = [
  ...
  },
  {
    ...
    settings: [
      ...
      { path: 'ingest.extractRelations', label: 'Extract relations during ingest', type: 'boolean',
        help: 'Usually off: the durable maintenance queue derives cross-document relations after facts become searchable. Turn on only to also extract same-document relations during entity enrichment.' },
      ...
      { path: 'ingest.workerConcurrency', label: 'Background workers', type: 'number', min: 1, max: 8, step: 1, restart: true,
        help: 'Concurrent durable ingestion/enrichment jobs. Searchable commits remain serialized and short.' },
    ],
  },
  {
    id: 'memory',
  ...
  },
  {
    id: 'jev',
    tier: 'pipeline',
    title: 'Jev enhancements',
    help: 'Jev is an optional remote decision layer that re-ranks the local shortlist and screens it for prompt injection. Sigil keeps the local order if Jev is disabled, unavailable, or uncertain. Save a key below, then enable it here when you are ready.',
    settings: [
      ...
      { path: 'jev.minScore', label: 'Minimum Jev relevance', type: 'number', min: 0, max: 1, step: 0.01,
        help: 'Only candidates at or above this Noul score move ahead of the local order. Calibrate this threshold against your own recall evaluation set.' },
      { path: 'jev.injectionMax', label: 'Drop facts above this injection score', type: 'number', min: 0, max: 1, step: 0.01,
        help: 'A stored fact that reads as an instruction to the agent rather than a memory is dropped from recall above this Noul score. Relevance only reorders; this is the one judgment that removes a fact.' },
      { path: 'jev.timeoutMs', label: 'Jev total request timeout (ms)', type: 'number', min: 1000, max: 60000, step: 1000,
        help: 'A whole-rerank budget shared by every candidate request, retries and backoff included. On timeout or provider failure, Sigil immediately keeps the local result order.' },
    ],
  },
  {
    id: 'engine',
    ...
    settings: [
      ...
      { path: 'llm.managedSession.poolSize', label: 'Pool size', type: 'number', min: 1, max: 8, step: 1, restart: true,
        help: 'Ingestion workers. Interactive and graph-maintenance work each have a separate worker so long enrichment cannot block live queries.' },
      ...
      { path: 'llm.managedSession.queueTimeoutMs', label: 'Queue timeout (ms)', type: 'number', min: 5000, max: 300000, step: 5000, restart: true,
        help: 'A task waiting this long without a worker falls back instead of hanging behind a booting, busy, or unhealthy pool.' },
