...
async function search(query, { namespaces, limit = 5, minConfidence = 'medium', useGraph = false, includeChunks = false, pointInTime, expand = false, route = true, categories, synthesize = config.search.synthesize, podScope = null, applyFloor = true, viewer = null, ctx = {} } = {}) {
  ...
  let result;
  if (matchedEntity) {
    result = await entityFirstSearch(matchedEntity, query, { namespaces, limit, minConfidence, includeChunks, pointInTime, categories, podIds, viewer });
  } else {
    result = await standardSearch(query, { namespaces, limit, minConfidence, useGraph, includeChunks, pointInTime, expand, categories, podIds, viewer });
  }
...
async function entityFirstSearch(entity, query, { namespaces, limit, minConfidence, includeChunks, pointInTime, categories, podIds, viewer }) {
  ...
  const [entityFacts, entityRelations, ...hybridResults] = await Promise.all([
    ...
    ...queryVariants.map((q, i) => coreHybridSearch(q, {
      ...
      namespaces, limit, minConfidence, includeChunks, pointInTime, categories, podIds, viewer,
    })),
  ]);
...
async function standardSearch(query, { namespaces, limit, minConfidence, useGraph, includeChunks, pointInTime, expand = false, categories, podIds, viewer }) {
  const queries = expand ? await expandQuery(query) : [query];
  const embeddings = await embedBatch(queries, { inputType: 'query' });

  const results = await Promise.all(
    queries.map((q, i) => coreHybridSearch(q, { queryEmbedding: embeddings[i], namespaces, limit, minConfidence, includeChunks, pointInTime, categories, podIds, viewer })),
  );
...
async function coreHybridSearch(query, { queryEmbedding: precomputed, namespaces, limit, minConfidence, includeChunks = false, pointInTime, categories, podIds, viewer }) {
  ...
  const factsPromise = hybridSearchFacts(query, queryEmbedding, {
    namespaces, limit, minConfidence, pointInTime, categories, podIds, viewer,
  });
